...
imagePath="/Users/admin/Desktop/all programs/my codes/traitement-image/imageATraiter/lena.png"
...
for i in range(0,imageColorer.shape[0]):
    for  j in range(0,imageColorer.shape[1]):
          histo[imageColorer[i,j]] +=1
print(histo)
plt.plot(histo)
plt.show()
#--------------------------------------Histogramme cumule--------------------------------------------
...
for  i in range(0,imageColorer.shape[0]):
    for j in range(0,imageColorer.shape[1]):
        imageColorer[i,j,0]=hc[imageColorer[i,j,0]]
        imageColorer[i,j,1]=hc[imageColorer[i,j,1]]
        imageColorer[i,j,2]=hc[imageColorer[i,j,2]]
cv.imshow("L'image egaliser",imageColorer)
cv.waitKey(0)
cv.destroyAllWindows()